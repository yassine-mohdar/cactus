...
@section('content')
    <div class="mb-6 flex items-center space-x-4">
        ...
        <h1 class="text-2xl font-bold tracking-tight text-ink">Add Product</h1>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <x-admin.card title="Basic Details">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="short_description" rows="3" class="w-full rounded-lg border border-border bg-white px-4 py-2 text-sm focus:border-sage focus:outline-none focus:ring-1 focus:ring-sage transition duration-150">{{ old('short_description') }}</textarea>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-ink mb-1">Full Description</label>
                            <textarea name="description" rows="8" class="w-full rounded-lg border border-border bg-white px-4 py-2 text-sm focus:border-sage focus:outline-none focus:ring-1 focus:ring-sage transition duration-150">{{ old('description') }}</textarea>
                        </div>
                    </div>
                </x-admin.card>
            ...
            <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="pt-2">
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" name="is_featured" value="1" @checked(old('is_featured')) class="rounded border-border text-sage focus:ring-sage">
                        ...
                        <div>
                            ...
                            <div class="max-h-64 overflow-y-auto w-full rounded-lg border border-border bg-white p-3 space-y-2">
                                @forelse($categories as $category)
                                    <label class="flex items-center space-x-2">
                                        <input type="checkbox" name="categories[]" value="{{ $category->id }}" @checked(is_array(old('categories')) && in_array($category->id, old('categories'))) class="rounded border-border text-sage focus:ring-sage">
                ...
                <x-admin.card title="Primary Media">
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-ink mb-1">Featured Image</label>
