...
<x-nino.page-header
    ...
    subtitle="Review payment activity, gateway state, and linked orders with clearer row-level actions." />
@if(session('success'))
    <x-nino.inline-alert tone="success" title="Transaction workflow updated" class="mb-4">
        {{ session('success') }}
    </x-nino.inline-alert>
@endif
...
<div class="mt-3 flex items-center gap-3">
    ...
    @if(request()->hasAny(['search','status','type','method','date_from','date_to']))<x-nino.button href="{{ route('admin.finance.transactions.index') }}" variant="outline">Clear</x-nino.button>@endif
</div>
</form>
    ...
    <table class="nino-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="px-4 py-3 text-xs text-[#61706B]">
                    @if($txn->order)
            ...
            <tr><td colspan="9" class="datatable-empty">
                        <div class="datatable-empty-panel">
                            <p class="text-sm font-semibold text-[#1E2B27]">No transactions found.</p>
                            <p class="text-sm text-[#61706B]">Try broader filters or a different reference search.</p>
                        </div>
                    </td></tr>
            @endforelse
        </tbody>
