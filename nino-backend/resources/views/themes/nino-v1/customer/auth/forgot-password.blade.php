...
<body class="min-h-screen bg-[#F4EFE6] font-sans text-slate-900">
    <div class="mx-auto flex min-h-screen max-w-4xl items-center justify-center px-4 py-10">
        <section class="w-full rounded-[28px] border border-[rgba(15,23,42,0.08)] bg-white/90 p-8 shadow-[0_30px_90px_-54px_rgba(15,23,42,0.35)] sm:p-10">
            <p class="text-[11px] font-bold uppercase tracking-[0.32em] text-slate-500">Customer recovery</p>
            <h1 class="mt-4 text-4xl font-extrabold tracking-[-0.04em] text-slate-900">Request a customer password reset</h1>
            <p class="mt-3 text-sm leading-6 text-slate-600">Enter the email address linked to your customer account and we’ll send a secure reset link.</p>
            ...
            <form method="POST" action="{{ route('customer.password.email') }}" class="mt-8 space-y-5">
                @csrf
                ...
                <button type="submit" class="inline-flex w-full items-center justify-center rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">Email reset link</button>
            </form>
        </section>
    </div>
</body>
