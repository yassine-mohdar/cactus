            ...
            @forelse($suppliers as $supplier)
                <tr class="group">
                    <td class="text-left">
                        ...
                        <div class="text-[10px] text-slate-500 font-mono tracking-widest mt-0.5">{{ $supplier->code ?? 'NO-CODE' }}</div>
                    </td>
                    <td class="text-left">
                        <div class="text-sm font-medium text-slate-600">{{ $supplier->email ?? 'No Email' }}</div>
                        <div class="text-xs text-slate-400 italic">{{ $supplier->phone ?? 'No Phone' }}</div>
                    </td>
                    <td class="text-center">
                        <div class="text-sm text-slate-900 font-semibold">{{ $supplier->city ?? 'N/A' }}</div>
                        <div class="text-[10px] text-slate-500 uppercase tracking-wider">{{ $supplier->country ?? 'N/A' }}</div>
                    </td>
                    <td class="text-center">
                ...
                <tr>
                    <td colspan="5" class="datatable-empty">
                        <div class="datatable-empty-panel">
                            <span class="material-symbols-outlined text-[2rem] text-slate-300">inventory_2</span>
                            <p class="text-sm font-semibold text-slate-900">No suppliers found.</p>
                            <p class="text-sm text-slate-500">Add your first supplier to start tracking purchasing partners.</p>
                        </div>
                    </td>
                </tr>
            @endforelse
