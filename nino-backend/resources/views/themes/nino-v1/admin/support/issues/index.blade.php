...
<div class="datatable-shell">
    ...
    <table class="nino-table">
        ...
        <tbody>
            ...
            <tr>
                <td class="px-4 py-3 font-mono text-xs text-slate-900 font-medium"><a href="{{ route('admin.support.issues.show', $issue) }}" class="table-link table-mono">{{ $issue->reference }}</a></td>
                <td class="px-4 py-3 text-sm text-slate-900">
                    <a href="{{ route('admin.support.issues.show', $issue) }}" class="table-link">{{ $issue->subject }}</a>
                    @if($issue->customer_name)<span class="text-xs text-slate-500 block">{{ $issue->customer_name }}</span>@endif
                </td>
                ...
                <td class="px-4 py-3 text-sm text-slate-500">{{ $issue->assignee?->first_name ?? 'Unassigned' }}</td>
                <td class="px-4 py-3 text-xs text-slate-500">{{ $issue->created_at->format('M d, H:i') }}</td>
            ...
            <tr><td colspan="8" class="datatable-empty">
                        <div class="datatable-empty-panel">
                            <p class="text-sm font-semibold text-slate-900">No issues found.</p>
                            <p class="text-sm text-slate-500">Try adjusting the filters or create a new issue.</p>
                        </div>
                    </td></tr>
            @endforelse
        </tbody>
