...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-slate-900 mt-2">Edit: {{ $method->name }}</h1>
</div>

@include('admin.shipping.methods.create')
@overwrite

{{-- This file re-uses the create form since $method is passed --}}
@extends('admin.layouts.app')
...
<form action="{{ route('admin.shipping.methods.update', $method) }}" method="POST" class="bg-white border border-slate-200 rounded-md shadow-sm p-6 max-w-2xl space-y-5">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <textarea name="description" rows="2" class="w-full rounded-lg border-slate-200 bg-background text-sm text-slate-900 shadow-sm focus:border-slate-900 focus:ring-slate-900/20">{{ old('description', $method->description) }}</textarea>
    </div>

    <div class="grid grid-cols-2 gap-4">
        <div>
            <label class="block text-sm font-semibold text-slate-900 mb-1">Base Cost (MAD) <span class="text-red-600">*</span></label>
            <input type="number" step="0.01" name="base_cost" value="{{ old('base_cost', $method->base_cost) }}" required class="w-full rounded-lg border-slate-200 bg-background text-sm text-slate-900 shadow-sm focus:border-slate-900 focus:ring-slate-900/20">
        </div>
        <div>
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="number" name="sort_order" value="{{ old('sort_order', $method->sort_order) }}" class="w-full rounded-lg border-slate-200 bg-background text-sm text-slate-900 shadow-sm focus:border-slate-900 focus:ring-slate-900/20">
        </div>
    </div>

    <div class="flex items-center gap-3">
        <input type="checkbox" name="is_enabled" value="1" id="is_enabled" {{ old('is_enabled', $method->is_enabled) ? 'checked' : '' }} class="rounded border-slate-200 text-slate-900 focus:ring-slate-900/20">
        <label for="is_enabled" class="text-sm font-medium text-slate-900">Enabled</label>
    </div>

    <div class="flex justify-end pt-2">
        <button type="submit" class="px-5 py-2 text-sm font-medium bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition-colors shadow-sm">Update Method</button>
    </div>
</form>
@endsection
