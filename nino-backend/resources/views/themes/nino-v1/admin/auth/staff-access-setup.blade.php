...
<body data-theme="{{ $adminTheme['key'] ?? 'nino-v1' }}" class="h-full font-sans antialiased text-ink bg-white flex flex-col items-center justify-center p-4">
    <div class="w-full max-w-md">
        <div class="text-center mb-8">
            ...
            <p class="mt-2 text-sm text-ink-muted">Set a password for your staff account.</p>
        </div>
        ...
        <div class="bg-white px-8 py-10 shadow-sm border border-border rounded-md">
            <form method="POST" action="{{ route('staff.access.setup.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email', $email) }}" required autofocus class="w-full rounded-lg border border-border bg-gray-50 px-4 py-2 text-sm text-gray-600 focus:outline-none" readonly>
                    @error('email')
                ...
                <div>
                    ...
                    <input id="password" type="password" name="password" required class="w-full rounded-lg border border-border bg-cream px-4 py-2.5 text-sm text-ink placeholder-ink-muted outline-none transition-colors focus:border-sage focus:ring-1 focus:ring-sage/30">
                    @error('password')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input id="password_confirmation" type="password" name="password_confirmation" required class="w-full rounded-lg border border-border bg-cream px-4 py-2.5 text-sm text-ink placeholder-ink-muted outline-none transition-colors focus:border-sage focus:ring-1 focus:ring-sage/30">
                </div>
