...
class ShippingMenu implements RegistersAdminMenu
{
    public function registerAdminMenu(MenuRegistry $registry): void
    {
        ...
        $registry->add(
            MenuItem::make('shipping.carriers')
                ->setLabel('Carriers')
                ->setIcon('<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 6.75V6a3 3 0 013-3h0a3 3 0 013 3v.75m-9 0h9m-9 0A2.25 2.25 0 003 9v8.25A2.25 2.25 0 005.25 19.5h13.5A2.25 2.25 0 0021 17.25V9a2.25 2.25 0 00-2.25-2.25m-9 0v3m6-3v3"></path></svg>')
                ->setParent('shipping_header')
                ->setRoute('admin.shipping.carriers.index')
                ->requirePermission(['shipping.viewAny', 'shipping.manage_carriers'])
                ->prioritizeForRoles([Role::SHIPPING_AGENT], 80)
                ->activeWhen('admin.shipping.carriers.*')
                ->setOrder(2)
        );
